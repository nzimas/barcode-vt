...
-- Voice toggles + random params in EDIT menu
...
for i=1,6 do
  ...
  params:add{type='binary',name='voice '..i..' toggle',id='voice_toggle_'..i,behavior='toggle',allow_pmap=true,action=function(v)
  if params:get("randomize_voices") == 1 then
    -- Allow manual toggling if randomize is off
    voice[i].enabled = (v == 1)
    if not voice[i].enabled then
      softcut.level(i, 0)
    else
      softcut.level(i, state.level * voice[i].level.calc)
    end
  else
    -- Ignore manual toggling if randomize is on
    print("Randomize voices is ON, ignoring manual toggle for voice "..i)
  end
end}
